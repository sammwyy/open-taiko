class SongSelect {
  constructor(fromTutorial, fadeIn, touchEnabled, songId, showWarning) {
    ...
    this.songs.sort((a, b) => {
      var catA =
        a.originalCategory in this.songSkin
          ? this.songSkin[a.originalCategory]
          : this.songSkin.default;
      var catB =
        b.originalCategory in this.songSkin
          ? this.songSkin[b.originalCategory]
          : this.songSkin.default;
    ...
    if (showCustom) {
      this.songs.push({
        title: assets.customSongs
          ? strings.customSongs.default
    ...
    if (
      p2.session ||
      (assets.customSongs && "customSelected" in localStorage)
    ) {
      fromTutorial = false;
    }
    ...
    var songIdIndex = -1;
    if (fromTutorial) {
      this.selectedSong = this.songs.findIndex(
        (song) => song.action === fromTutorial
      );
      this.playBgm(true);
    } else {
      if (songId) {
      ...
      if (songIdIndex !== -1) {
        ...
        this.selectedSong = Math.min(
          Math.max(0, assets.customSelected),
          this.songs.length - 1
        );
      } else if ((!p2.session || fadeIn) && "selectedSong" in localStorage) {
        this.selectedSong = Math.min(
          Math.max(0, localStorage["selectedSong"] | 0),
          this.songs.length - 1
        );
      }
      if (!this.showWarning) {
    ...
    if ("selectedDiff" in localStorage) {
      this.selectedDiff = Math.min(
        Math.max(0, localStorage["selectedDiff"] | 0),
        this.diffOptions.length + 3
      );
    }
    ...
    if (this.state.showWarning) {
      ...
      if (name === "confirm") {
        if (this.selectedDiff === 0) {
          ...
          this.toLoadSong(
            this.selectedDiff - this.diffOptions.length,
            shift,
            ctrl
          );
        }
  ...
  mouseDown(event) {
    if (
      event.target === this.selectable ||
      event.target.parentNode === this.selectable
    ) {
  ...
  mouseMove(event) {
    ...
    if (this.state.showWarning) {
      ...
      if (
        ...
        if (
          moveTo === null &&
          this.state.moveHover === 0 &&
          !this.songs[this.selectedSong].courses
        ) {
  ...
  songSelMouse(x, y) {
    if (
      this.state.locked === 0 &&
      this.songAsset.marginTop <= y &&
      y <= this.songAsset.marginTop + this.songAsset.height
    ) {
      x -= 1280 / 2;
      var dir = x > 0 ? 1 : -1;
      x = Math.abs(x);
      ...
      var moveBy =
        Math.ceil(
          (x - selectedWidth / 2 - this.songAsset.marginLeft / 2) /
  ...
  diffSelMouse(x, y) {
    if (this.state.locked === 0) {
      if (223 < x && x < 367 && 132 < y && y < 436) {
        ...
        this.songs[this.selectedSong].maker &&
        this.songs[this.selectedSong].maker.id > 0 &&
        this.songs[this.selectedSong].maker.url &&
        x > 230 &&
        x < 485 &&
        y > 446 &&
        y < 533
        ...
        var moveBy =
          Math.floor((x - 550) / ((1050 - 550) / 5)) + this.diffOptions.length;
        var currentSong = this.songs[this.selectedSong];
        if (
          (this.state.ura && moveBy === this.diffOptions.length + 3) ||
          currentSong.courses[
  ...
  moveToSong(moveBy, fromP2) {
    ...
    if (p2.session && !fromP2) {
      ...
      if (
        this.songs[this.selectedSong].courses &&
        !this.songs[this.selectedSong].unloaded &&
        (this.state.locked === 0 || fromP2)
      ) {
        this.state.moveMS = ms;
      } else {
        this.state.moveMS =
          ms - this.songSelecting.speed * this.songSelecting.resize;
      }
      ...
      for (var i = 0; i < Math.abs(moveBy) - 1; i++) {
        this.playSound(
          "se_ka",
          (resize + i * soundsDelay) / 1000,
          fromP2 ? fromP2.player : false
        );
      }
      this.pointer(false);
    }
  ...
  toOptions(moveBy) {
    if (!p2.session) {
      ...
      this.selectedDiff = 1;
      do {
        this.state.options = this.mod(
          this.optionsList.length,
          this.state.options + moveBy
        );
      } while (
        (p2.socket.readyState !== 1 || assets.customSongs) &&
  ...
  redraw() {
    ...
    if (this.winW !== winW || this.winH !== winH) {
      ...
      var songsLength =
        Math.ceil(
          winW / ratio / (this.songAsset.width + this.songAsset.marginLeft)
        ) + 1;
      ...
      this.selectTextCache.resize(
        (textW + 53 + 60 + 1) * 2,
        this.songAsset.marginTop + 15,
        ratio + 0.5
      );
        ...
        this.sessionCache.set(
          ...
          (ctx) => {
            this.draw.layeredText(
              ...
              [{ outline: "#000", letterBorder: 8 }, { fill: "#fff" }]
            );
          }
        );
      }
    ...
    if (screen === "title" || screen === "titleFadeIn") {
      if (ms > this.state.screenMS + 1000) {
        ...
        this.state.moveMS =
          ms -
          this.songSelecting.speed * this.songSelecting.resize +
          (ms - this.state.screenMS);
        this.state.locked = 3;
    ...
    if (
      (screen === "song" || screen === "difficulty") &&
      ((this.showWarning && !this.showWarning.shown) ||
        scoreStorage.scoreSaveFailed)
    ) {
      if (!this.showWarning) {
        ...
        (ctx) => {
          if (category) {
            ...
            this.draw.layeredText(
              ...
              [
                {
                  outline: selectedSong.skin.outline,
                  letterBorder: 12,
                  shadow: [3, 3, 3],
                },
                { fill: "#fff" },
    ...
    if (screen === "song") {
      ...
      if (
        this.state.catJump ||
        (this.state.move && ms > this.state.moveMS + resize2 - scrollDelay)
      ) {
        ...
        if (!isJump) {
          ...
          this.selectedSong = this.mod(
            this.songs.length,
            this.selectedSong + this.state.move
          );
        } else {
          ...
          if (this.state.move > 0) {
            var nextSong = this.songs.find(
              (song) =>
                this.mod(this.songs.length, this.songs.indexOf(song)) >
                  currentIdx &&
                song.category !== currentCat &&
                song.canJump
            );
            if (!nextSong) {
            ...
            var isFirstInCat =
              this.songs.findIndex((song) => song.category === currentCat) ==
              this.selectedSong;
            if (!isFirstInCat) {
              var nextSong = this.songs.find(
                (song) =>
              ...
              for (; idx >= 0; idx--) {
                if (
                  this.songs[idx].category !== lastCat &&
                  this.songs[idx].action !== "back"
                ) {
      ...
      if (this.state.moveMS && ms < this.state.moveMS + changeSpeed) {
        xOffset =
          (Math.min(scroll, Math.max(0, elapsed - resize - scrollDelay)) /
            scroll) *
          (this.songAsset.width + this.songAsset.marginLeft);
        xOffset *= -this.state.move;
        if (elapsed < resize) {
          selectedWidth =
            this.songAsset.width +
            ((resize - elapsed) / resize) *
              (selectedWidth - this.songAsset.width);
        ...
        do {
          if (hasUra && this.state.move > 0) {
            ...
            if (this.selectedDiff > this.diffOptions.length + 4) {
              ...
              if (this.state.ura) {
                this.selectedDiff =
                  previousSelection === this.diffOptions.length + 3
                    ? this.diffOptions.length + 4
                    : previousSelection;
                break;
              } else {
                this.state.move = -1;
            ...
            this.selectedDiff = this.mod(
              this.diffOptions.length + 5,
              this.selectedDiff + this.state.move
            );
          }
        } while (
          (this.selectedDiff >= this.diffOptions.length &&
            !currentSong.courses[
              this.difficultyId[this.selectedDiff - this.diffOptions.length]
            ]) ||
          (this.selectedDiff === this.diffOptions.length + 3 &&
            this.state.ura) ||
    ...
    if (songSelMoving) {
      ...
      screen !== "title" &&
      screen !== "titleFadeIn" &&
      ms > this.state.moveMS + 100
    ) {
      if (
        this.previewing !== "muted" &&
        this.previewing !== this.selectedSong &&
        "id" in this.songs[this.selectedSong]
      ) {
        this.startPreview();
      }
    }
    ...
    if (screen === "title" || screen === "titleFadeIn" || screen === "song") {
      ...
      for (var i = this.selectedSong + 1; ; i++) {
        var _x =
          winW / 2 +
          (i - this.selectedSong - 1) *
            (this.songAsset.width + this.songAsset.marginLeft) +
          this.songAsset.marginLeft +
          selectedWidth / 2 +
          xOffset;
        if (_x > winW) {
          startFrom = i - 1;
    ...
    this.draw.songFrame({
      ...
      disabled:
        p2.session && currentSong.action && currentSong.action !== "random",
      innerContent: (x, y, w, h) => {
        ...
        if (
          ...
          var opened =
            (selectedWidth - this.songAsset.width) /
            (this.songAsset.selectedWidth - this.songAsset.width);
          var songSel = true;
        } else {
          var textW = strings.id === "en" ? 350 : 280;
          this.selectTextCache.get(
          ...
          for (var i = 0; i < this.diffOptions.length; i++) {
            ...
            if (
              this.diffOptions[i].iconName === "options" &&
              (this.selectedDiff === i || this.state.options !== 0)
            ) {
        ...
        var drawDifficulty = (ctx, i, currentUra) => {
          if (currentSong.courses[this.difficultyId[i]] || currentUra) {
            ...
            if (p2.session) {
              score[p2.player === 1 ? "push" : "unshift"](
                scoreStorage.getP2(currentSong.hash, false, true)
              );
            }
            var reversed = false;
            for (var a = players; a--; ) {
              ...
              if (!reversed && players === 2 && p === 1 && crownType) {
                ...
                this.draw.crown({
                  ...
                  type: crownType,
                  x:
                    (songSel ? x + 33 + i * 60 : x + 402 + i * 100) +
                    (players === 2 ? (p === 0 ? -13 : 13) : 0),
                  y: songSel ? y + 75 : y + 30,
                  scale: 0.25,
            ...
            var songStarsObj = currentUra
              ? currentSong.courses.ura
              : currentSong.courses[this.difficultyId[i]];
            ...
            var elapsedMS =
              this.state.screenMS > moveMS || !songSel
                ? this.state.screenMS
                : moveMS;
            ...
            if (!songSel) {
              ...
              if (currentDiff === i && !this.touchEnabled) {
                this.draw.diffCursor({
                  ...
                  y: _y - 65,
                  side:
                    currentSong.p2Cursor === currentDiff &&
                    p2.socket.readyState === 1,
                  two: p2.session && p2.player === 2,
                });
              }
        ...
        for (var i = 0; currentSong.courses && i < 4; i++) {
          var currentUra =
            i === 3 &&
            ((this.state.ura && !songSel) ||
              (currentSong.courses.ura && songSel));
        ...
        if (hasMaker || currentSong.lyrics) {
          if (songSel) {
              ...
              this.draw.layeredText(
                {
                  ...
                  x: _x + 68,
                  y:
                    _y + (strings.id === "ja" || strings.id === "en" ? 25 : 28),
                  width: 172,
                },
                [{ outline: "#fff", letterBorder: 6 }, { fill: "#000" }]
              );
            } else {
              this.draw.layeredText(
                {
            ...
            currentSong.maker &&
            currentSong.maker.id > 0 &&
            currentSong.maker.name
          ) {
            var _x = x + 62;
            var _y = y + 380;
            ctx.lineWidth = 5;
            ...
            this.draw.layeredText(
              ...
              [{ outline: "#fff", letterBorder: 8 }, { fill: "#000" }]
            );
        ...
        if (!songSel && currentSong.courses.ura) {
          ...
          var _y = y + 258;
          ctx.fillStyle =
            "rgba(0, 0, 0, " + 0.2 * this.draw.easeInOut(1 - fade) + ")";
          ctx.beginPath();
    ...
    if (!p2.session || p2.player === 1) {
      ...
      var rank =
        account.loggedIn || !gameConfig.accounts || p2.session
          ? false
          : strings.notLoggedIn;
    } else {
    ...
    if (p2.session) {
      if (p2.player === 1) {
        ...
        var name = account.loggedIn
          ? account.displayName
          : strings.default2PName;
      }
      this.nameplateCache.get(
        {
          ctx: ctx,
    ...
    if (this.state.showWarning) {
      ...
      ctx.fill();
      var layers = [{ outline: "#000", letterBorder: 10 }, { fill: "#fff" }];
      this.draw.layeredText(
        {
          ctx: ctx,
  ...
  drawClosedSong(config) {
    ...
    if (
      "p2Cursor" in config.song &&
      config.song.p2Cursor !== null &&
      p2.socket.readyState === 1
    ) {
      this.draw.diffCursor({
        ctx: ctx,
  ...
  drawSongCrown(config) {
    if (!config.song.action && config.song.hash) {
      ...
      if (p2.session) {
        score[p2.player === 1 ? "push" : "unshift"](
          scoreStorage.getP2(config.song.hash, false, true)
        );
      }
      for (var i = this.difficultyId.length; i--; ) {
        ...
        for (var p = players; p--; ) {
          ...
          if (
            config.song.courses[this.difficultyId[i]] &&
            score[p][diff] &&
            score[p][diff].crown
          ) {
            this.draw.crown({
    ...
    if ("id" in currentSong) {
      ...
      if (songObj) {
        ...
        if (currentSong.previewMusic) {
          ...
          var promise = snd.previewGain
            .load(currentSong.previewMusic)
            .catch(() => {
  ...
  getUnloaded(selectedSong, songObj, currentId) {
    ...
    return file
      .read(currentSong.type === "tja" ? "sjis" : "")
      .then((data) => {
      ...
      .then(() => {
        ...
        var index = assets.songs.findIndex(
        ...
        this.state.moveMS =
          this.getMS() - this.songSelecting.speed * this.songSelecting.resize;
  ...
  addSong(song) {
    ...
    var subtitle = this.getLocalTitle(
    ...
    if (song.category_id !== null && song.category_id !== undefined) {
      var category = assets.categories.find(
        (cat) => cat.id === song.category_id
      );
      var categoryName = this.getLocalTitle(
        category.title,
        category.title_lang
      );
  ...
  startP2() {
    ...
    pageEvents.add(p2, "message", (response) => {
      ...
      if (
        p2.session &&
        (response.type == "songsel" || response.type == "catjump")
      ) {
  ...
  playSound(id, time, snd) {
    if (
      !this.drumSounds &&
      (id === "se_don" || id === "se_ka" || id === "se_cancel")
    ) {
      return;
    }
  ...
  clean() {
    ...
    pageEvents.remove(loader.screen, [
      "mousemove",
      "mouseleave",
      "mousedown",
      "touchstart",
